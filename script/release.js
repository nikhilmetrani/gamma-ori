...
function zipAssets () {
  ...
  const zipAssets = [{
    name: 'gamma-ori-mac.zip',
    path: path.join(outPath, 'Electron API Demos-darwin-x64', 'Electron API Demos.app')
  }, {
    name: 'gamma-ori-windows.zip',
    path: path.join(outPath, 'Gamma-Ori-win32-ia32')
  }, {
    name: 'gamma-ori-linux.zip',
  ...
  return Promise.all(zipAssets.map(zipAsset)).then((zipAssets) => {
    return zipAssets.concat([{
      ...
      name: `Gamma-Ori-${version}-full.nupkg`,
      path: path.join(outPath, 'windows-installer', `Gamma-Ori-${version}-full.nupkg`)
    }])
  })
}
